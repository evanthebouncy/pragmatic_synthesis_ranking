from src.prag_utils import *
from src.animals import make_animalgame
from src.prag_ordering import find_ordering, make_Lo
...
def make_L0(M, H_prior):
    ...
    def L0(us):
        if str(us) in L0_cache:
            return L0_cache[str(us)]
        ret = []
        ...
        if sum(ret) == 0:
            ret1 = ret
        L0_cache[str(us)] = ret1
        
        return L0(us)
    return L0

S1_sofar_h = {}
def compute_alt_terms(U, U_prior, L0, utts_sofar, h, u):
    the_key = str((U, U_prior, L0, utts_sofar, h, u))
    if the_key in S1_sofar_h:
        return S1_sofar_h[the_key]
    else:
        alt_terms = []
        ...
        S1_sofar_h[the_key] = alt_terms
        return compute_alt_terms(U, U_prior, L0, utts_sofar, h, u)
...
def make_S1(L0, U_prior, U):
    ...
    def S1(us, h):
        ...
        for u in us:
            ...
            # alt_terms = []
            alt_terms = compute_alt_terms(U, U_prior, L0, utts_sofar, h, u)
            # for uu in range(len(U)):
            #     uu_prior = U_prior[uu]
            #     u_together = utts_sofar + [uu]
            #     pl0 = L0(u_together)[h]
...
def getL1(H, U, M, H_prior, U_prior):
    L0 = make_L0(M, H_prior)
    S1 = make_S1(L0, U_prior, U)
    S1_sample = make_S1_sampler(L0, U_prior, U)
    L1 = make_L1(L0,S1,H)
...
def getL1L0(H, U, M, H_prior, U_prior):
    L0 = make_L0(M, H_prior)
    S1 = make_S1(L0, U_prior, U)
    S1_sample = make_S1_sampler(L0, U_prior, U)
    L1 = make_L1(L0,S1,H)
...
def run_inc(H, U, M, H_prior, U_prior):
    # H, U, M = make_linegame()
